"""
Predict band gaps with ALIGNN on the JARVIS vacancy_db (defective cells).

Mirrors slako_v07_vacancy's filter (Z <= 65). The atoms field is `defective_atoms`
(NOT `atoms`). vacancy_db has no DFT band gap reference — only `ef` (formation
energy of the defect).

Outputs: results/alignn_predictions.json
  Each entry: {id, formula, alignn_bandgap}  (+ ef if present)
"""
