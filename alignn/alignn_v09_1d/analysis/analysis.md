...
Pearson correlation between |ALIGNN - DFT| and `e_form`: **-0.365**. Negative: less stable structures have smaller error (often because they trend metallic in DFT and ALIGNN agrees). Plot at `error_vs_eform.png`.
...
Cross-method comparison vs SlakoNet on the same matched structures: `slakonet/slako_v09_1d/analysis/sk_vs_alignn.png` and `confusion_sk_vs_alignn.png`. Metrics in `slakonet/slako_v09_1d/analysis/sk_vs_alignn_metrics.csv`. Cross-dataset roll-up at `slakonet/sk_vs_alignn_cross_dataset.csv`.
