"""
Predict formation energy (eV/atom) with ALIGNN on Alexandria alex_supercon
candidates.

Mirrors slako_v08_supercon's filter (Z <= 65). The dataset has no DFT
formation-energy reference — it carries electron-phonon descriptors (Tc, λ,
dos(Ef), wlog, Debye-T) instead. ALIGNN's predicted formation energy is a
cross-method output for SK-vs-ALIGNN comparison; not parity-comparable.

Outputs: results/alignn_eform_predictions.json
  Each entry: {id, formula, alignn_eform_per_atom, Tc, la, dosef, wlog, debye}
"""
