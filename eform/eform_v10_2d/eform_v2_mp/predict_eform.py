"""
Predict formation energy (eV/atom) with ALIGNN on Alexandria PBE 2D.

Mirrors slako_v10_2d's filter (Z <= 65). Note slako_v10's filter checks
`i['elements']` at the top level (not `atoms.elements`); we replicate that here
so the ALIGNN run covers exactly the same set SlakoNet did. Falls back to
`atoms.elements` when the top-level key is missing. The dataset's `e_form`
(PBE) is the parity reference; this ALIGNN model was trained on JARVIS
OptB88vdW e_form, so any bias is partly a functional shift, not pure model
error.

Outputs: results/alignn_eform_predictions.json
  Each entry: {id, formula, band_gap_ind, band_gap_dir, e_form, alignn_eform_per_atom}
"""
...
INPUT_ZIP = "alexandria_pbe_2d_2024.10.1_jarvis_tools.json.zip"
...
def load_entries(path):
    ...
    for i in tqdm(data, desc="Filtering"):
        ...
        if atoms is None:
            continue
        elements = i.get('elements') or atoms.get('elements', [])
        if not all(e in ALLOWED_SYMBOLS for e in elements):
            continue
        valid.append(i)
...
def main():
    ...
    for entry in entries:
        ...
        for k in ("band_gap_ind", "band_gap_dir", "e_form"):
            if k in entry:
                out[k] = entry[k]
        output.append(out)
