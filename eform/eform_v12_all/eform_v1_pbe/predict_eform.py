"""
Predict formation energy (eV/atom) with ALIGNN on the full Alexandria PBE 3D
dataset matching slako_v12_all: NO e_above_hull filter, NO Z<=65 filter.

Sharded via SLURM array. Reads SLURM_ARRAY_TASK_ID and resolves SHARD_COUNT
from TOTAL_SHARDS first (single-shard resubmits like `sbatch --array=9` set
SLURM_ARRAY_TASK_COUNT=1 and would slice entries[9::1] = the entire 4.5M-entry
tail; pass `--export=ALL,TOTAL_SHARDS=100` to force the original sharding).
Each shard writes its own checkpoint dir and final per-shard output JSON.

The dataset's `e_form` (PBE) is the parity reference; this ALIGNN model was
trained on JARVIS OptB88vdW e_form, so any bias is partly a functional shift,
not pure model error.

Outputs: results/alignn_eform_predictions_shard{NNN}.json
  Each entry: {mat_id, formula, band_gap_ind, band_gap_dir, e_form, e_above_hull, alignn_eform_per_atom}
"""
...
CHECKPOINT_DIR = f"results/alignn_eform_checkpoints_shard{SHARD_ID:03d}"
OUTPUT_FILE = f"results/alignn_eform_predictions_shard{SHARD_ID:03d}.json"
        ...
        if fname.endswith(".json"):
            with open(os.path.join(CHECKPOINT_DIR, fname), "r") as f:
                for entry in json.load(f):
                    done[entry["mat_id"]] = entry["alignn_eform_per_atom"]
...
def main():
            ...
            for dat in tqdm(test_loader, desc="ALIGNN inference"):
                ...
                for pred_val in preds:
                    ...
                    results_chunk.append({
                        "mat_id": jid,
                        "alignn_eform_per_atom": pred_val,
                    })
    ...
    for entry in entries:
        ...
        output.append({
            ...
            "alignn_eform_per_atom": done[mid],
        })
