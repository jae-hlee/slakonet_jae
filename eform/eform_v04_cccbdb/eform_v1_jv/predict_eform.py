"""
Predict formation energy (eV/atom) with ALIGNN on the CCCBDB molecule set.

Mirrors slako_v04_cccbdb's filter (Z <= 65) so the ALIGNN run covers the same
structures SlakoNet was run on. CCCBDB is molecular — the JARVIS-trained
formation-energy ALIGNN model was trained on crystal periodic graphs, so
predictions on isolated molecules are out-of-distribution and should be
treated as a sanity bound only. CCCBDB also has no DFT formation-energy
reference (only molecular thermochemistry), so this run is a cross-method
output for SK/ALIGNN comparison.

Outputs: results/alignn_eform_predictions.json
  Each entry: {jid, formula, alignn_eform_per_atom}  (+ auto-propagated scalars)
"""
...
def main():
    ...
    for entry in entries:
        ...
        # heavy structured fields so the output JSON stays small.
        SKIP = {ATOMS_KEY, "dos_values", "dos_energies", "geometry_out",
                "lattice_mat", "coords", "elements"}
        out = {ID_KEY: sid, "alignn_eform_per_atom": done[sid]}
