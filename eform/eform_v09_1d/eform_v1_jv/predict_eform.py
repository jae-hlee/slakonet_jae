"""
Predict formation energy (eV/atom) with ALIGNN on Alexandria PBE 1D.

Mirrors slako_v09_1d's filter (Z <= 65; element check on atoms.elements).
1D structures have two vacuum directions; ALIGNN's k-NN graph builds
regardless. The dataset's `e_form` (PBE) is the parity reference; this
ALIGNN model was trained on JARVIS OptB88vdW e_form, so any bias is
partly a functional shift, not pure model error.
...
INPUT_ZIP = "alexandria_pbe_1d_2024.10.1_jarvis_tools.json.zip"
