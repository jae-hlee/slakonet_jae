"""
Predict formation energy (eV/atom) with ALIGNN on the JARVIS vacancy_db
(defective cells).

Mirrors slako_v07_vacancy's filter (Z <= 65). The atoms field is `defective_atoms`
(NOT `atoms`).

vacancy_db carries `ef`, but `ef` is the **vacancy formation energy (eV per
defect)** — energy cost of creating the vacancy in a host crystal — which is
conceptually distinct from the per-atom formation energy ALIGNN predicts (energy
of the defective cell vs elemental references, in eV/atom). DO NOT parity-compare
ALIGNN's `alignn_eform_per_atom` to `ef` directly. The output is useful for
SK-vs-ALIGNN cross-method comparison only.

Outputs: results/alignn_eform_predictions.json
  Each entry: {id, formula, alignn_eform_per_atom}  (+ ef carried for cross-ref)
"""
...
INPUT_ZIP = "vacancydb.json.zip"
OUTPUT_FILE = "results/alignn_eform_predictions.json"
CHECKPOINT_DIR = "results/alignn_eform_checkpoints"
BATCH_SIZE = 32
NUM_WORKERS = 0
CHECKPOINT_EVERY = 500

ID_KEY = "id"
ATOMS_KEY = "defective_atoms"
        ...
        if fname.endswith(".json"):
            with open(os.path.join(CHECKPOINT_DIR, fname), "r") as f:
                for entry in json.load(f):
                    done[entry[ID_KEY]] = entry["alignn_eform_per_atom"]
...
def main():
    ...
    for entry in entries:
        ...
        out = {
            ...
            "alignn_eform_per_atom": done[sid],
        }
        # `ef` is vacancy formation energy (eV/defect), NOT the per-atom
        # formation energy ALIGNN predicts. Carried for cross-ref only.
        if "ef" in entry:
            out["ef"] = entry["ef"]
        output.append(out)
