"""
Predict formation energy (eV/atom) with ALIGNN on the JARVIS surface_db slabs.

Mirrors slako_v06_surface's filter (Z <= 65). The dataset uses `name` as the id
field (NOT `jid`). Slab geometry has a vacuum direction; ALIGNN's k-NN graph
builds regardless of cell shape.

Outputs: results/alignn_eform_predictions.json
  Each entry: {name, formula, alignn_eform_per_atom}  (+ surf_cbm/surf_vbm if present)
"""
...
INPUT_ZIP = "surface_db_dd.json.zip"
...
def main():
    ...
    for entry in entries:
        ...
        for k in ("surf_cbm", "surf_vbm", "scf_cbm", "scf_vbm"):
            if k in entry:
                out[k] = entry[k]
        output.append(out)
