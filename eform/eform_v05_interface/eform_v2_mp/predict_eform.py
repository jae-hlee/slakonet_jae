"""
Predict formation energy (eV/atom) with ALIGNN on the JARVIS interface_db slabs.

Mirrors slako_v05_interface's filter (Z <= 65) so the ALIGNN run covers the same
structures SlakoNet was run on. Slab geometry includes a vacuum direction;
ALIGNN's k-NN graph builds regardless of cell shape.

Outputs: results/alignn_eform_predictions.json
  Each entry: {jid, formula, alignn_eform_per_atom}  (+ any DFT e_form fields the dataset carries)
"""
...
INPUT_ZIP = "interface_db_dd.json.zip"
