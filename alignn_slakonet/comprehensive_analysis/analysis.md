...
evaluated in `alslak_v01_pbe`, `alslak_v02_mbj`, `alslak_v03_opt` against the
SlakoNet DFTB predictions from `alslak_v00`. All numbers, plots, and CSVs in
...
|--------|-------------------|--------:|-------|
| `alslak_v00/results/sk_scalars.json`           | **SlakoNet** (TBLite/DFTB scalar band-gap) | 31,211 | shared DFTB baseline for all ALIGNN variants |
| `alslak_v01_pbe/results/alignn_predictions.json` | **ALIGNN `mp_gappbe_alignn`** (trained on MP PBE gaps)          | 48,764 | PBE-matched |
| `alslak_v02_mbj/results/alignn_predictions.json` | **ALIGNN `jv_mbj_bandgap_alignn`** (trained on JARVIS TB-mBJ gaps) | 48,764 | different functional → gaps systematically *opened* |
| `alslak_v03_opt/results/alignn_predictions.json` | **ALIGNN `jv_optb88vdw_bandgap_alignn`** (trained on OptB88vdW gaps) | 48,764 | vdW-GGA, gaps slightly *closed* vs PBE |
