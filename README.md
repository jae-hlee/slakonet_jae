...
alignn_slakonet/              ALIGNN cross-check on Alexandria PBE 3D
  alslak_v00/                 Mirror of slako_v03_alex for the combined study
  alslak_v01_pbe/             ALIGNN  mp_gappbe_alignn       (label-matched)
  alslak_v02_mbj/             ALIGNN  jv_mbj_bandgap_alignn  (TB-mBJ)
  alslak_v03_opt/             ALIGNN  jv_optb88vdw_bandgap_alignn
...
From `alignn_slakonet/comprehensive_analysis/`. Reference is Alexandria PBE indirect gap.
...
python alignn_slakonet/comprehensive_analysis/compare_all.py
```
...
- **Element support.** SlakoNet ships Slater–Koster parameters for Z ≤ 65 (up to terbium); heavier elements are filtered out up front. For Alexandria PBE 3D, the combined Z ≤ 65 + `e_above_hull == 0` filter reduces the 4,489,295-entry dataset to 48,764 structures (see `alignn_slakonet/alslak_v01_pbe/alignn_1282176.out`). The nominal Z ≤ 65 ceiling is optimistic: on Alexandria 2D, every one of the 8,000 "missing" entries (~9% of valid targets) turned out to contain an f-block lanthanide (Ce–Tb, Z = 58–65), and inference silently fails inside `gpu_worker`. The effective usable ceiling is **Z ≤ 57** (through La); full analysis in `slakonet/slako_v10_2d/analysis/analysis.md`.
...
- **DOS broadening is fixed.** `σ = 0.1 eV` is hardcoded inside `SimpleDftb.calculate_dos`. The `slako_v08_supercon/sigma_diagnostic.py` script shows how to monkey-patch it for sensitivity testing.
