...
def run_inference(prepared, model, shell_dict, device):
    ...
    return {
        ...
        'e_form': item['e_form'],
        'e_above_hull': item['e_above_hull'],
        'sk_bandgap': bandgap,
...
def gpu_worker(gpu_id, chunk, shell_dict):
    # CUDA must initialize fresh in this spawn child. The parent deliberately
    # avoids calling any torch.cuda.* so the driver state here is clean.
...
def run_multi_gpu(valid_entries, shell_dict, num_gpus):
...
if __name__ == '__main__':
    ...
    # Detect GPU count from SLURM env instead of torch.cuda.device_count().
    # Calling torch.cuda.* in the parent before spawn workers causes the
    ...
    if num_gpus > 1:
        ...
        #   SLAKO_CPU_WORKERS=128 python jslako_v12.py
        # For 5M entries, manually shard to keep parent mem bounded:
        #   SLURM_ARRAY_TASK_ID=0 SLURM_ARRAY_TASK_COUNT=20 python jslako_v12.py
