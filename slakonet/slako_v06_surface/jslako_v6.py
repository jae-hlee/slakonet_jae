...
def run_inference(prepared, model, shell_dict, device):
    ...
    return {
        ...
        'scf_vbm': item['scf_vbm'],
        'scf_cbm': item['scf_cbm'],
        'sk_bandgap': bandgap,
...
def run_single_gpu(valid_entries, shell_dict):
    ...
    for item in tqdm(valid_entries, desc="Processing"):
        try:
            fname = os.path.join("results", f"{item['name']}.json")
            ...
            try:
                ...
                n_prep_fail += 1
                tqdm.write(f"PREP FAIL: {item['name']} {item['formula']} — {e}")
                continue
            ...
            if info == "TIMEOUT":
                n_timeout += 1
                tqdm.write(
                    f"TIMEOUT ({TIMEOUT_SECONDS}s): {item['name']} — skipping (timeouts so far: {n_timeout})"
                )
                continue
            if info:
                tqdm.write(
                    f"{info['name']} {info['formula']} vbm={info['scf_vbm']} cbm={info['scf_cbm']} sk={info['sk_bandgap']:.4f} n={len(mem)}"
                )
                dumpjson(data=info, filename=fname)
                mem.append(info)
        except Exception as e:
            tqdm.write(f"ERROR: {item['name']} — {e}")
    ...
    for item in tqdm(chunk, desc=f"GPU {gpu_id}", position=gpu_id):
        try:
            ...
            if info == "TIMEOUT":
                n_timeout += 1
                tqdm.write(
                    f"[GPU {gpu_id}] TIMEOUT ({TIMEOUT_SECONDS}s): {item['name']} — skipping (timeouts: {n_timeout})"
                )
                continue
            if info:
                tqdm.write(
                    f"[GPU {gpu_id}] {info['name']} {info['formula']} vbm={info['scf_vbm']} cbm={info['scf_cbm']} sk={info['sk_bandgap']:.4f}"
                )
                dumpjson(data=info, filename=fname)
                results.append(info)
        except Exception:
...
if __name__ == '__main__':
    ...
    import zipfile
    local_zip = 'surface_db_dd.json.zip'
    print(f"Loading from local file: {local_zip}")
    with zipfile.ZipFile(local_zip) as z:
        with z.open('surface_db_dd.json') as f:
            dft_3d = json.load(f)
    ...
    for i in tqdm(dft_3d, desc="Filtering entries"):
        if not all_elements_supported(i['atoms']['elements']):
            skipped_elements += 1
            continue
        fname = os.path.join(RESULTS_DIR, f"{i['name']}.json")
        if os.path.exists(fname):
            skipped_existing += 1
            continue
