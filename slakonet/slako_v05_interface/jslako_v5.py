...
def run_inference(prepared, model, shell_dict, device):
    ...
    return {
        'jid': item['jid'],
        'optb88vdw_bandgap': item['optb88vdw_bandgap'],
        'optb88vdw_cbm': item['optb88vdw_cbm'],
        'optb88vdw_vbm': item['optb88vdw_vbm'],
...
def run_single_gpu(valid_entries, shell_dict):
    ...
    for item in tqdm(valid_entries, desc="Processing"):
        try:
            fname = os.path.join("results", f"{item['jid']}.json")
            ...
            try:
                ...
                n_prep_fail += 1
                tqdm.write(f"PREP FAIL: {item['jid']} {item['species']} — {e}")
                continue
            ...
            if info == "TIMEOUT":
                n_timeout += 1
                tqdm.write(
                    f"TIMEOUT ({TIMEOUT_SECONDS}s): {item['jid']} — skipping (timeouts so far: {n_timeout})"
                )
                continue
            if info:
                tqdm.write(
                    f"{info['jid']} bg={info['optb88vdw_bandgap']:.4f} sk={info['sk_bandgap']:.4f} n={len(mem)}"
                )
                dumpjson(data=info, filename=fname)
                mem.append(info)
        except Exception as e:
            tqdm.write(f"ERROR: {item['jid']} — {e}")
    ...
    for item in tqdm(chunk, desc=f"GPU {gpu_id}", position=gpu_id):
        try:
            ...
            if info == "TIMEOUT":
                n_timeout += 1
                tqdm.write(
                    f"[GPU {gpu_id}] TIMEOUT ({TIMEOUT_SECONDS}s): {item['jid']} — skipping (timeouts: {n_timeout})"
                )
                continue
            if info:
                tqdm.write(
                    f"[GPU {gpu_id}] {info['jid']} bg={info['optb88vdw_bandgap']:.4f} sk={info['sk_bandgap']:.4f}"
                )
                dumpjson(data=info, filename=fname)
                results.append(info)
        except Exception:
...
if __name__ == '__main__':
    ...
    import zipfile
    local_zip = 'interface_db_dd.json.zip'
    print(f"Loading from: {local_zip}")
    with zipfile.ZipFile(local_zip) as z:
        with z.open('interface_db_dd.json') as f:
            dft_3d = json.load(f)
    ...
    # Filter: all elements Z <= 65, and json doesn't already exist
    valid_entries = []
    skipped_existing = 0
    skipped_elements = 0
    for i in tqdm(dft_3d, desc="Filtering entries"):
        if not all_elements_supported(i['atoms']['elements']):
            skipped_elements += 1
            continue
        fname = os.path.join(RESULTS_DIR, f"{i['jid']}.json")
        if os.path.exists(fname):
            skipped_existing += 1
            continue
