...
def run_single_gpu(valid_entries, shell_dict):
    ...
    for item in tqdm(valid_entries, desc="Processing"):
        try:
            ...
            if info:
                tqdm.write(
                    f"{info['jid']} {info['species']} homo={info['homo']} lumo={info['lumo']} sk={info['sk_bandgap']:.4f} n={len(mem)}"
                )
...
def gpu_worker(gpu_id, chunk, shell_dict):
    ...
    for item in tqdm(chunk, desc=f"GPU {gpu_id}", position=gpu_id):
        try:
            ...
            if info:
                tqdm.write(
                    f"[GPU {gpu_id}] {info['jid']} {info['species']} homo={info['homo']} lumo={info['lumo']} sk={info['sk_bandgap']:.4f}"
                )
                dumpjson(data=info, filename=fname)
                results.append(info)
        except Exception:
