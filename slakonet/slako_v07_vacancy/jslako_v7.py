...
def prepare_inputs(item):
    atoms = Atoms.from_dict(item['defective_atoms'])
...
def run_inference(prepared, model, shell_dict, device):
    ...
    return {
        ...
        'defective_atoms': item['defective_atoms'],
    }
...
def run_single_gpu(valid_entries, shell_dict):
    ...
    for item in tqdm(valid_entries, desc="Processing"):
        try:
            fname = os.path.join("results", f"{item['id']}.json")
            ...
            try:
                ...
                n_prep_fail += 1
                tqdm.write(f"PREP FAIL: {item['id']} — {e}")
                continue
            ...
            if info == "TIMEOUT":
                n_timeout += 1
                tqdm.write(
                    f"TIMEOUT ({TIMEOUT_SECONDS}s): {item['id']} — skipping (timeouts so far: {n_timeout})"
                )
                continue
            if info:
                tqdm.write(
                    f"{info['id']} ef={info['ef']} sk={info['sk_bandgap']:.4f} n={len(mem)}"
                )
    ...
    for item in tqdm(chunk, desc=f"GPU {gpu_id}", position=gpu_id):
        try:
            ...
            if info == "TIMEOUT":
                n_timeout += 1
                tqdm.write(
                    f"[GPU {gpu_id}] TIMEOUT ({TIMEOUT_SECONDS}s): {item['id']} — skipping (timeouts: {n_timeout})"
                )
                continue
            if info:
                tqdm.write(
                    f"[GPU {gpu_id}] {info['id']} ef={info['ef']} sk={info['sk_bandgap']:.4f}"
                )
                dumpjson(data=info, filename=fname)
                results.append(info)
        except Exception:
...
if __name__ == '__main__':
    ...
    local_zip = 'vacancydb.json.zip'
    print(f"Loading from: {local_zip}")
    with zipfile.ZipFile(local_zip) as z:
        with z.open('vacancydb.json') as f:
            dft_3d = json.load(f)
    ...
    for i in tqdm(dft_3d, desc="Filtering entries"):
        if not all_elements_supported(i['defective_atoms']['elements']):
            skipped_elements += 1
            continue
        fname = os.path.join(RESULTS_DIR, f"{i['id']}.json")
        if os.path.exists(fname):
            skipped_existing += 1
            continue
